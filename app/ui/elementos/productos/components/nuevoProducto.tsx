...
import { insertarInventario } from "../../../../../src/store/reducer";
...
interface DetalleProducto {
	...
	stock_disponible: number | undefined;
	stock_alerta: number | undefined;
	estado_producto: string;
	nombre_categoria: string;
	nombre_marca: string;
	descripcion_producto: string;
	url_producto: undefined | string | File;
}
...
export function AgregarNuevoProducto({
	...
	setIsNewProduct: Dispatch<SetStateAction<boolean>>,
}) {
	...
	const [valueImagen, setValueImagen ] = useState<File | string | undefined>(
	...
	const uploadFile = (file: File): Promise<string> => {
		return new Promise((resolve, reject) => {
			if (!file) {
				reject("Debe seleccionar un archivo.");
				return;
			}
			...
			reader.onload = (event: ProgressEvent<FileReader>) => {
				...
				const rawLog = result.split(',')[1]; // Extrae los datos en base64
	
				fetch('https://script.google.com/macros/s/AKfycbzf8cGlPR6l7P7kACBx8YeLXzvGrJEZU3QZQ9n0cKRjqrm7D3pXWSwcClYn6Q2J1ZRwKQ/exec', {
					method: "POST",
					body: JSON.stringify({
				...
				.then(data => {
					if (data && data.url) {
						...
						reject("La respuesta no contiene una URL.");
					}
				})
				.catch(error => reject("Hubo un problema al enviar el archivo. Inténtalo de nuevo."));
			};
	...
	function handleImageChange(e: React.ChangeEvent<HTMLInputElement>) {
		console.log("Nuevo valor recibido:", e.target.files?.[0]);
		console.log(typeof e.target.files?.[0]);
	
		setValueImagen(e.target.files?.[0]);
	}

	const confirmar = async () => {
		if (!seleccionarProducto || !valueImagen) return;
		setLoading(true);
		const imagenxd =  await uploadFile(valueImagen as File);
		const productoActualizado : DetalleProducto = {
			...
			url_producto: imagenxd,
		};
		console.log(productoActualizado);
		try {
			const response = await fetch("/api/inventario/insertarProducto", {
				method: "POST",
				headers: {
			...
			dispatch(insertarInventario(data));
	...
	const [valueIdMarcaText, setValueIdMarcaText] = useState(seleccionarProducto.nombre_marca); */}
							...
							<Input
								...
								size="sm"
								type="file"
								isInvalid={validImagen}
								label="Imagen del producto"
								placeholder="Suba la Imagen del Proucto"
